...
function Get-PageContext {
    ...
    if ($isRoot) {
        $links = @{
            ...
            CreatorLogo = "assets/images/site/myLogo.jpg"
        }
    }
    elseif ($isNested) {
        $links = @{
...
<div class="w-full max-w-xl border-t border-outline-variant/50 pt-5 mb-5 flex flex-col sm:flex-row items-center justify-center gap-3" data-footer-credit>
<img alt="Logo KKN-PPM UGM Unit YO-031" class="w-12 h-12 sm:w-14 sm:h-14 rounded-full object-cover border border-outline-variant shadow-sm shrink-0" decoding="async" loading="lazy" src="$($links.CreatorLogo)">
<div class="text-center sm:text-left">
<div class="font-label-sm text-label-sm uppercase tracking-wider text-on-surface-variant mb-1">Dibuat oleh</div>
<div class="font-label-md text-label-md font-semibold text-primary dark:text-primary-fixed leading-snug">KKN-PPM UGM 2026 PERIODE 2 UNIT YO-031</div>
</div>
</div>
